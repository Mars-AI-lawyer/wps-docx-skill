...
class RedlineWriterTest(unittest.TestCase):
    ...
    def test_author_can_be_overridden(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            insertions = parts["word/document.xml"].xpath(".//w:ins", namespaces=NS)
            comments = parts["word/comments.xml"].xpath(".//w:comment", namespaces=NS)
            log = json.loads(log_path.read_text(encoding="utf-8"))
            report = validate_docx(output, log_path, expected_author="QA Reviewer")
            self.assertEqual(log["author"], "QA Reviewer")
            self.assertTrue(report["valid"], report)
            self.assertTrue(deletions)
            self.assertTrue(insertions)
            self.assertTrue(comments)
            self.assertTrue(
                all(node.get(qn("w", "author")) == "QA Reviewer" for node in deletions)
            )
            self.assertTrue(
                all(node.get(qn("w", "author")) == "QA Reviewer" for node in insertions)
            )
            self.assertTrue(
                all(node.get(qn("w", "author")) == "QA Reviewer" for node in comments)
            )
